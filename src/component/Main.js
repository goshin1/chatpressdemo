    ...
    return <div>
        ...
        <div id="mainDiv">
            ...
            <div id="chatRemocon">
                ...
                <div className="remoconIcon" onClick={() => {
                    let res = [
                        {user_icon_path : require('../img/chatpressIcon.png'), user_nickname : 'member1'},
                        {user_icon_path : require('../img/chatpressIcon.png'), user_nickname : 'member2'},
                        {user_icon_path : require('../img/chatpressIcon.png'), user_nickname : 'member3'},
                        {user_icon_path : require('../img/chatpressIcon.png'), user_nickname : 'member4'},
                        {user_icon_path : require('../img/chatpressIcon.png'), user_nickname : 'member5'},
                    ]
                    let temp = [];
                    for(let i = 0; i < res.length; i++){
                        temp.push(
                ...
                <div className="remoconIcon" onClick={() => {
                    let temp = [];
                    let blank = [
                        {document_id : 0, document_name : '문서1', share_code : '1111'},
                        {document_id : 1, document_name : '문서2', share_code : '1111'},
                        {document_id : 2, document_name : '문서3', share_code : '1111'},
                    ]
                    for(let i = 0; i < blank.length; i++){
                        temp.push(<div className="documentBlock" onClick={() => {
                            // 해당 문서의 공유 기능 만들기
                        }}>
                            {blank[i].document_name}

                            <input type="button" className="shareBtn" style={{background : "rgb(250,10,10)"}} value="삭제" onClick={() => {
                                if(window.confirm(`${blank[i].document_name} 문서를 삭제하실껀가요?`) === false) return
                                alert("삭제되었습니다.");
                                setUpload("none");
                                setPopupType()
                                // axios.get("/document/delete?documentId="+blank[i].document_id, {
                                //     headers : {
                                //         "access" : cookies.get("access")
                                //     }
                                // }).then((response) => {
                            ...
                            <input type="button" className="shareBtn" style={{background : "rgb(84, 216, 84)"}} value="공유" onClick={() => {
                                // axios.get("/share/load?documentId="+blank[i].document_id, {
                                //     headers : {
                                //         "access" : cookies.get("access")
                                //     }
                                // }).then((response) => {
                                ...
                                //     axios.post("/chat/insert", {
                                //         chat_user_nickname : profile.user_nickname,
                                //         chat_message : `<a href=${"http://211.188.51.27:8080/#/create/"+blank.share_code}>문서 공유</a>`,
                                //         chat_type : "text",
                                //         room_id : num
                                //     }, {
                                //         headers : {
                                ...
                                //             chatRoomId : select,
                                //             type : "SEND",
                                //             message : `${"http://211.188.51.27:8080/#/create/"+blank.share_code}`,
                                //             user : profile.user_nickname,
                                //             fileType : "text"}));
                                //     }).catch((error) => {
                                //         if(error.response.data === "access token expired"){
